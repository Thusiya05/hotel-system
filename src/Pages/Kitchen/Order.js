...
function ViewOrderDerails(props){
    ...
    useEffect(() => {    
        // console.log("hello");
        axios.get(`/order/findByoderNumber/${props.ordersDetails}`)
        .then(res => {
...
function AssignStewardToOrder(props){
    ...
    useEffect(() => {    
        // console.log("hello");
        axios.get(`/api/v1/avaialableStewards`)
        .then(res => {
            // let order = res.data;
...
function Order() {
    ...
    function PrepareOrder(orderId){
        axios.post(`/order/prepareOrder/${orderId}`);
        setStatus2(!status2);
    ...
    useEffect(() => {
        axios.get('/order/inprogressOrders')
        .then(res => {
